...
export class VerificationService {
  ...
  async processRequest(
    ...
    const newStatus = processDto.approved
  ...
  async getStats() {
    const [pending, approved, rejected] = await Promise.all([
      this.verificationModel
        .countDocuments({ status: RequestStatus.PENDING })
        .exec(),
      this.verificationModel
        .countDocuments({ status: RequestStatus.APPROVED })
        .exec(),
      this.verificationModel
        .countDocuments({ status: RequestStatus.REJECTED })
        .exec(),
    ]);
