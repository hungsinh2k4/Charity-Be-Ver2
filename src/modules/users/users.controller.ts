...
export class UsersController {
    ...
    @UseGuards(JwtAuthGuard)
    @ApiBearerAuth()
    @ApiOperation({ summary: 'Request user verification (submit identity document and selfie)' })
    @ApiResponse({ status: 200, description: 'Verification request submitted successfully' })
    @ApiResponse({ status: 400, description: 'Already verified or verification already pending' })
    ...
    @Roles(Role.ADMIN)
    @ApiBearerAuth()
    @ApiOperation({ summary: '[Auditor] Get all users with pending verification' })
    @ApiResponse({ status: 200, description: 'Returns list of users pending verification' })
    ...
    @Roles(Role.ADMIN)
    @ApiBearerAuth()
    @ApiOperation({ summary: '[Auditor] Get user verification details including documents' })
